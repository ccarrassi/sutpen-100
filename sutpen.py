...
class UserPage(Handler):
	def get(self, user):
		...
		content = self.pager(page_num=page, prop='author', val=user)
...
class TagPage(Handler):
	...
	def get(self, tag):
		...
		if not page:
			page = 0
		route = '/tag/%s?p=%s' % (tag, str(int(page)-1)), '/tag/%s?p=%s' % (tag, str(int(page)+1))
		entries = self.pager(page_num=page, prop='tags', val=tag)

		self.render("content.html", entries=entries, page=page, route=route)
